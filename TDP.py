...
<head>
    ...
    <style>
        * { box-sizing: border-box; }
        ...
        #map { height: 300px; width: 100%; border-radius: 8px; border: 1px solid #334155; }
        .control-panel {
            margin-top: 8px;
            padding: 10px 12px;
            background: #0f172a;
...
<body>
    ...
    <div class="control-panel">
        <div style="flex: 1; min-width: 200px;">
            <div>📍 Đang chọn: <strong id="coord-text" style="color: #4ade80;">10.734728, 106.663666</strong></div>
            <div id="status-msg" style="font-size: 11px; color: #94a3b8; margin-top: 2px;">(Bấm nút dưới để lấy GPS hoặc copy)</div>
        </div>
        <div class="btn-group">
...
# ==================== BỘ TỰ ĐỘNG BÓC TÁCH TỌA ĐỘ TỪ GOOGLE MAPS ====================
def parse_google_maps_input(raw_text: str):
    """
    Tự động trích xuất cặp tọa độ (lat, lng) từ bất kỳ chuỗi văn bản nào:
    - 10.734728, 106.663666
    - Link Google Maps: https://www.google.com/maps?q=10.734728,106.663666
    - Link @: https://www.google.com/maps/@10.734728,106.663666,17z
    - Text có chứa chữ kèm tọa độ
    """
    if not raw_text:
        return None

    raw_text = urllib.parse.unquote(str(raw_text))

    # Tìm mẫu tọa độ số thập phân dạng (lat, lng)
    # Vĩ độ VN thường từ 8 đến 24, kinh độ từ 102 đến 110
...
try:
    ...
    # ==================== NHẬP LIỆU & NÚT DÁN GOOGLE MAPS ====================
    st.markdown("##### 📍 Nhập hoặc Dán vị trí từ Google Maps")

    # Quản lý giá trị tọa độ trong session_state
    if "current_coord_input" not in st.session_state:
        st.session_state.current_coord_input = "10.734728, 106.663666"

    # Giao diện nút Dán nhanh từ bộ nhớ tạm bằng JavaScript
    ...
    <div style="display: flex; gap: 8px; margin-bottom: 6px;">
        <button id="paste-btn" style="
            ...
            align-items: center;
            gap: 6px;
        ">📥 Dán nhanh vị trí vừa copy từ Google Maps</button>
        <span id="paste-status" style="font-size: 12px; color: #94a3b8; align-self: center;"></span>
    </div>
    ...
    document.getElementById('paste-btn').addEventListener('click', async function() {
        ...
        try {
            ...
            if (!text || text.trim() === '') {
                status.innerText = "Bộ nhớ tạm đang trống!";
                status.style.color = "#f87171";
                return;
            }
            // Tìm ô input của Streamlit ở trang cha
            var parentInputs = window.parent.document.querySelectorAll('input[type="text"]');
            var targetInput = null;
            for (var i = 0; i < parentInputs.length; i++) {
                if (parentInputs[i].placeholder && parentInputs[i].placeholder.includes("Ví dụ: 10.734728")) {
            ...
            if (targetInput) {
                ...
                status.innerText = "✅ Đã dán thành công!";
                status.style.color = "#4ade80";
            } else {
                navigator.clipboard.writeText(text);
                status.innerText = "Hãy bấm phím Ctrl+V (hoặc Chạm giữ -> Dán) vào ô bên dưới";
                status.style.color = "#38bdf8";
            }
        } catch (err) {
            status.innerText = "Trình duyệt chặn đọc clipboard. Bạn vui lòng dán thẳng vào ô bên dưới nhé!";
            status.style.color = "#facc15";
        }
    });
    </script>
    """
    st.components.v1.html(paste_component_html, height=45, scrolling=False)

    col_input1, col_input2 = st.columns([3, 1])
    with col_input1:
        raw_coord = st.text_input(
            "Tọa độ hoặc Link Google Maps:",
            value=st.session_state.current_coord_input,
            placeholder="Ví dụ: 10.734728, 106.663666 hoặc dán link Google Maps vào đây",
            help="Hỗ trợ dán trực tiếp link Google Maps hoặc tọa độ"
        )
    with col_input2:
    ...
    # Nút bấm tính khoảng cách
    if st.button("🚀 Tính khoảng cách", type="primary", use_container_width=True):
        parsed_coord = parse_google_maps_input(raw_coord)

        if parsed_coord is not None:
            input_lat, input_lng = parsed_coord
            st.session_state.current_coord_input = f"{input_lat:.6f}, {input_lng:.6f}"

            with st.spinner(f'Đang tính toán từ tọa độ: {input_lat:.6f}, {input_lng:.6f}...'):
                ...
                st.markdown(f"### 🎯 Kết quả 5 trạm gần nhất (Vị trí: `{input_lat:.6f}, {input_lng:.6f}`)")
            ...
            st.error("❌ Không nhận dạng được tọa độ từ nội dung bạn dán! Vui lòng kiểm tra lại link hoặc văn bản đã copy.")
